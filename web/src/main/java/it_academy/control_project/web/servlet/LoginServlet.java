...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        WebUtils.forward("login", request, response);
        /*it_academy.control_project.data.university.it_academy.control_project.data.AuthorizationUser user = securityService.login(login, password);
        if(user == null){
            request.setAttribute("error", "Login or password invalid");
            it_academy.control_project.web.servlet.WebUtils.forword("login", request, response);
        }
        assert user != null;
        log.info("user {} logged", user.getLogin());
        request.getSession().setAttribute("authorizationUser", user);
        it_academy.control_project.web.servlet.WebUtils.redirect("/user", request, response);*/
    }
}
